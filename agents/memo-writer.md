...
- Paths to skills: `skills/legal-memo-prose-style/SKILL.md`, `skills/legal-memo-docx-render/SKILL.md`.
...
- Paths to house-style and legal-memo-docx-render skills.
...
Structure depends on the chosen template — read `templates/<template_id>.md` carefully and follow its required sections. **All templates share one rhetorical surface**: the four-beat Risk subsection pattern from `skills/legal-memo-prose-style/SKILL.md`. Templates vary in section depth and length, not in tone or per-subsection shape.
...
- **Heading discipline self-check (hard rule, blocking at review).** All headings (H1 title, H2 numbered sections, H3 sub-issues) are noun phrases — not questions, not imperatives. Hierarchy: H1 → H2 → H3 only; no H4 in analytical or surface sections; no skip jumps (H2 directly to H4). If a sub-issue needs further breakdown, fold it into paragraph structure within the H3 subsection. `style-reviewer` enforces. See `skills/legal-memo-prose-style/SKILL.md` §Heading discipline.
...
- **House style**: read `legal-memo-prose-style/SKILL.md` and apply (anti-patterns, confidentiality, language conventions). Specifically the §Sentence structure Hard limits subsection — the 40-word / 2-idea threshold is not optional. Other points:
